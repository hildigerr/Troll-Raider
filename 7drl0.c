...
#include "7drl0func4.h"

#define MAX_ITEM_WINDOWS 3
#define MAX_WINDOW_POINTERS_NEEDED 3
...
int main( int argc, char* argv[] )
{
	...
	MYBOOL run, need_more_cmd, done_with_sub;
//	char* dat_file_name = "scores.dat";
//	FILE* indata;
	char* nam = "Skallagrim\n";
//	char nam[RT_SUB_COLS - 2];
	WINDOW *display_btm, *display_right, *cmd_list[MAX_WINDOW_POINTERS_NEEDED];
	ITEM* itmptr;
...
////	myflush(stdin);
	...
	pc.attdir =	story_line( INTRO, NULL );//Actually displays only after program termination
	...
	/* INITIALIZE CURRENT LEVEL IF NEEDED */
		if(curlv[pc.maplv].is_new == ONN)
		{
			...
			/* Get Race and Class */
				...
				if( getp_itm( &pc.inventory[0], 1, 1 ) == FAIL ) return( error("ERROR: Failed to get starting equipment",2) );//SHOULD GIVE PC AN AXE
	...
	/* TAKE TURN */
		...
		/* PROCESS CMD */
			...
			/* MOVE assess legality */
				...
				else if( ACTIVE_LOCATION.is_wall == ONN ) {say("You bumped into a wall!");}//Currently Takes a Turn
			/* MOVE possibilities */
				...
				else if( ACTIVE_LOCATION.is_door == ONN )
				{
					if( pc.maplv == HVILLAGE )
					{
						...
						if( toupper( getch() ) == 'N' ) need_more_cmd.bbool = ONN;
						else{ /* Assume YES */
				...
				{ 
					if( ACTIVE_LOCATION.is_ustair == ONN ) {say("You found some ascending stairs!");}
					...
					else if( ACTIVE_LOCATION.litter.is_ == ONN ) {say("You have found an item!");}
					move(pc.locr,pc.locc);
...
//				else say("Oh ok, then we wont kick you out of here.");
			...
			/* Equipment *//* RHS */
				...
				for(i=0; i < MAX_SLOTS; i++ )
				{
					...
					else error("ERROR: Slot machine error!", i );
				}/* end MAX_SLOTS for */
			...
			/* SUB CMD */
				done_with_sub.bbool = OFF;
				do{
				/* Get Input */
//					if( cmd == EQUIPMENT );
//					else if( cmd == REMOVE_ITM );
//					else if( cmd == DESTROY_ITEM );
					if( ( cmd > INVENTORY )&&( cmd < DESTROY_ITEM ) );/*All i sub cmds accessed directly in this range*/
				...
				/* Process sub cmd */
							...
							else switch( slot_of(itmptr) )
							{
								...
								case ARM: /* must equip to ARM slot *///TODO:NEEDS TESTING
									if( pc.equip[slot_of(itmptr)] != NULL )
									{
										...
										if( toupper(getch()) == 'Y' )
										{
											pc.equip[slot_of(itmptr)]->is_equipped = OFF;//Unequip old item
											itmptr->is_equipped = ONN;
								...
								case OFF: /* These are all 2handed weapons *///TODO
									if( itmptr->is_2handed == OFF ) error("Error: Strange Item!", itmptr->type );
								...
								default: error("ERROR: Hit switch default!", slot_of(itmptr)); /* ERROR */
							}/* end slot switch */
						}/* end is_equipable if */
					...
					{
					/* Select Slot */
						do{say("Destroy which item? ");}while( (cmd = get_uitem()) == NOT_PLACED );
						...
						/* Check for item existence @ slot location */
							...
							error("You broke it!", FAIL);
					}/* end DESTROY_ITEM if */
					else if( cmd == DROP_ITEM )
					{
					/* Select Slot */
						do{say("Drop which item? ");}while( (cmd = get_uitem()) == NOT_PLACED );
						...
						/* Check for item existence @ slot location */
							...
							/* Drop item */
								...
								if( ACTIVE_LOCATION.litter.is_ == OFF )
								{
					...
					{
						need_more_cmd.bbool = ONN;
						done_with_sub.bbool = ONN;
					}/* end canceled else */
				}while(done_with_sub.bbool == OFF);/* end sub cmd */
			...
			{
#ifdef DEBUG_OLD
				cmd_list[0] = newwin(MAX_ROW,(BTM_SUB_COLS-RT_SUB_COLS),0,0);
				if( cmd_list[0] == NULL ){ endwin(); return ("ERROR: No Cmd List Window!", cmd); }
				for(i=0; i < MAX_LV_ITM; i++)
				{
					if( wprintw(cmd_list[0],"\n %d %s [%d,%d] %d",i,curlv[pc.maplv].itm[i].name,curlv[pc.maplv].itm[i].stats[0],curlv[pc.maplv].itm[i].stats[1], curlv[pc.maplv].itm[i].worth) == ERR )
						return( error("ERROR: wprintw err", curlv[pc.maplv].itm[i].type));
	
				}/* end for each item */
				box(cmd_list[0],'|','+');
				overlay(cmd_list[0],stdscr);
				wrefresh(cmd_list[0]);
		...
		}while( need_more_cmd.bbool == ONN );/* end need more cmd do */
